...
float pidCtrl::calcColorSenagaPid(){
  float p,i,d;
  rgb_raw_t rgb;
  colorSensor.getRawColor(rgb);
  int color_val = rgb.r + rgb.g +rgb.b;
  ...
  p = Kp_color * diff_color_walk[1];
  i = Ki_color * integral;
  d = Kd_color * (diff_color_walk[1] - diff_color_walk[0]) / delta_t;
...
void pidCtrl::setTargetBrightness(int setTarget){
  target_val = setTarget;
}

void pidCtrl::setTargetColor(int setTarget){
  target_val_color = setTarget;
}
